...
$(document).ready(function() {
    const productRegistryContractAddress = '0xF94e8c36DF3989F9403A0aFeD50aF0c0C66126cE';
    ...
    $('#contractLink').attr('href', 'https://ropsten.etherscan.io/address/' + productRegistryContractAddress); // 홈에서 전자주소가 나오는 부분
    ...
    async function itemUploadButton() {
		...
		let account = selectedAddress // 갯수
		...
		contract.addProStru(howMany, productName, whereIs, function(err, result) { // 스마트컨트랙에 저장
			if (err)
				return showError("Smart contract call failed: " + err);
			showInfo(`Document ${result} <b>successfully added</b> to the registry.`);
		}); 
